...
`;
const siteSettings = `*[_type == "siteSettings"] [0] {...}`;
const allPostsFields = `
  ...
  posttype,
  defined(youtube) => {'youtube_url': youtube.url}
...
export async function getCovid19(preview) {
  const results = await getClient(preview).fetch(
    `*[_type == "page"][title == "Covid 19 Dashboard"][0]`,
  );
  return results;
}
...
export async function getSlug() {
  const results = await client.fetch(`*[_type == "page"]{ 'slug': slug.current }`);
  return results;
}
...
export async function getHeaderTitle(preview) {
  const results = await getClient(preview).fetch(`*[_type == "page" ] {
    defined(headerTitle) =>{ headerTitle},'slug': slug.current
    }`);
